...
public partial class Parents_Message : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("select b.ExamDate, a.Fclass,b.SID,b.Examaccesscode,concat(c.FirstName,' ',c.LastName) as Name,'You will need SID and examaccesscode to write exams, also note the exam date, exam must be taken that date' as Information from D_Classes a,D_Examschedule b,D_Students c where a.ClassID = b.ExamClass and c.SID = b.SID and b.ExamStatus='P' and c.Status='WE' and Parent_ID='"+Session["PID"].ToString()+"'"))
            {
