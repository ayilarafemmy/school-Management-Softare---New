...
public partial class admin_students : System.Web.UI.Page
{
    ...
    protected void BindGrid4()
    {
        ...
        {
            con.Open();
            string com = "select distinct ClassID, FClass as Class from D_Classes where ClassID in (select class from D_Students where Parent_ID='"+Session["PID"].ToString()+"')";
    ...
    //    {
    //        using (SqlCommand cmd = new SqlCommand("select a.SID,concat(a.FirstName,' ',a.LastName) as Name,CONCAT('~/Students/',a.Photo,a.filetype) as Photo,b.Fclass as Class,a.Gender,a.Balance,a.Status from D_Students a, D_Classes b where a.Class = b.ClassID and a.Parent_ID='"+Session["PID"].ToString()+"'"))
    //        {
    ...
    protected void OnRowUpdating(object sender, GridViewUpdateEventArgs e)
{
    ...
    protected void LinkButton2_Click(object sender, EventArgs e)
    {
        ...
        using (StringWriter sw = new StringWriter())
        {
            using (HtmlTextWriter hw = new HtmlTextWriter(sw))
            {
                ...
                pdfDoc.Close();
                Response.ContentType = "application/pdf";
                Response.AddHeader("content-disposition", "attachment;filename=Subjects_" + DropDownList4.SelectedItem.Text +DateTime.Now.ToString("ddMMyy_hhmm")+".pdf");
                Response.Cache.SetCacheability(HttpCacheability.NoCache);
                Response.Write(pdfDoc);
                Response.End();
            }
    ...
    {
        string q = "select SubjectID,Subject,Teacher,PassMark,TotalMark,Type from D_subjects where ClassID='"+DropDownList4.SelectedValue+"'";
        string n = "Subjects_" + DropDownList4.SelectedItem.Text + DateTime.Now.ToString("ddMMyyhhmm");
        export(q, n);
    }
...
if(DropDownList4.SelectedItem.Text == "Select Class")
...
else
        {
            ...
            string query = "select SubjectID,Subject,Teacher,PassMark,TotalMark,Type from D_subjects where ClassID='"+DropDownList4.SelectedValue+"'";
            BindGridMethod(query);
            //if doesn't exist, show message and return
            if(GridView1.Rows.Count < 1)
            {
                MessageBox("No Subject Found");
                GridView1.Visible = false;
                LinkButton1.Visible = false;
                LinkButton2.Visible = false;
                return;
