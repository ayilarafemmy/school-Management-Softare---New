...
public partial class Teacher_Students : System.Web.UI.Page
{
    ...
    protected void BindGrid4()
    {
        ...
        {
            con.Open();
            string com = "select distinct ClassID, FClass as Class from D_Classes where Teacher='"+Session["StaffID"].ToString()+"';";
    ...
    {
        string q = "select SID,concat(Photo,filetype) as photo,CONCAT(FirstName,' ',LastName) as Name,Gender,Roll_No,Bio, Balance,Status  FROM D_Students  where Status !='R' and class='" + DropDownList4.SelectedValue + "'";
        string n = "Students_" + DropDownList4.SelectedItem.Text + DateTime.Now.ToString("ddMMyyhhmm");
        export(q, n);
    }
    ...
    protected void DropDownList4_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string query = "select SID,concat('~/Students/',Photo,filetype) as photo,CONCAT(FirstName,' ',LastName) as Name,Gender,Roll_No,Bio, Balance,Status  FROM D_Students  where Status !='R' and class='" + DropDownList4.SelectedValue + "'";
            BindGridMethod(query);
            //if doesn't exist, show message and return
            if (GridView1.Rows.Count < 1)
            {
                MessageBox("No Student Found");
                GridView1.Visible = false;
                LinkButton1.Visible = false;
                LinkButton2.Visible = false;
                return;
