...
public partial class admin_students : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("select PID,concat(First_Name,' ',Last_Name) as Name,CONCAT('~/Parents/',identity_Upload,filetype) as Photo,Email,Phone,Status from D_Parents"))
            {
    ...
    protected void OnRowUpdating(object sender, GridViewUpdateEventArgs e)
{
    ...
    using (SqlConnection con = new SqlConnection(constr))
    {
        using (SqlCommand cmd = new SqlCommand("UPDATE D_Parents SET Phone = @Phone, Email = @Email,Status = @Status WHERE PID = @PID"))
        {
    ...
    protected void OnRowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        using (SqlConnection con = new SqlConnection(constr))
        {
            using (SqlCommand cmd = new SqlCommand("DELETE FROM D_Parents WHERE PID = @PID"))
            {
                cmd.Parameters.AddWithValue("@PID", PID);
                cmd.Connection = con;
                con.Open();
    ...
    protected void LinkButton2_Click(object sender, EventArgs e)
    {
        ...
        using (StringWriter sw = new StringWriter())
        {
            using (HtmlTextWriter hw = new HtmlTextWriter(sw))
            {
                ...
                pdfDoc.Close();
                Response.ContentType = "application/pdf";
                Response.AddHeader("content-disposition", "attachment;filename=Parents_"+DateTime.Now.ToString("ddMMyy_hhmm")+".pdf");
                Response.Cache.SetCacheability(HttpCacheability.NoCache);
                Response.Write(pdfDoc);
                Response.End();
            }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        //search and populate
        string jj = "select PID,concat(First_Name,' ',Last_Name) as Name,CONCAT('~/Parents/',identity_Upload,filetype) as Photo,Email,Phone,Status from D_Parents where PID='"+ TextBox1.Text+ "' OR First_Name like '%"+ TextBox2.Text+ "%' OR Last_Name like '%" + TextBox2.Text + "%' OR email='"+TextBox3.Text+"' OR phone='"+TextBox4.Text+"'";
        BindGridMethod(jj);
