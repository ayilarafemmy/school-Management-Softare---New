...
public partial class Admin_Dashboard : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Image1.ImageUrl = ConfigurationManager.AppSettings["InstitutionLogo"];
            //    string qq = "Select StaffID, FirstName, LastName, Phone,Email,Role, SecondaryRole,FullAddress,CreateDate,Password,Gender,DOB,Bio as Comment from D_Staff where StaffID='"+ Session["StaffID"].ToString() + "'";
            //ListViewBind(qq);
            //string qqa = "Select Notice from D_Notices where Status='A' order by NID desc";
            // GridViewBind(qqa);
            string ki = "Select top 8 Notice as Annoucements from D_Notices where Status='A' order by NID desc";
            GridViewBind4(ki);
        ...
        sda3.Fill(ds3, "D_Account");
            ...
            //Label1.Text = kk.ToString("N", separator);
           ...
           // if(k1.ToString() !="")
           // {
           //     Decimal k1a = Decimal.Parse(k1);
           //     Label3.Text = k1a.ToString("N", separator);
           // }
           //else
           // {
           //     Label3.Text = "0";
           // }
            ...
            //    Label4.Text = eea.ToString("N", separator);
            //}
            //else
            //{
            //    Label4.Text = "0";
            ...
            cmd8a.CommandText = "select profile,FirstName as Name,Concat(FirstName,' ',LastName) as FullName,SecondaryRole from D_Staff where StaffID='" + Session["StaffID"].ToString() + "'";
            cmd8a.Connection = con;
            sda8a.SelectCommand = cmd8a;
            sda8a.Fill(ds8a, "D_Staff");
            string jj = ds8a.Tables[0].Rows[0]["profile"].ToString();
            Label11.Text = ds8a.Tables[0].Rows[0]["Name"].ToString();          
            Image2.ImageUrl = "~/Staff/" + jj+"";

            cmd8b.CommandText = "select sum(CAST(Balance AS INT)) as bal from D_Students";
            cmd8b.Connection = con;
            sda8b.SelectCommand = cmd8b;
            sda8b.Fill(ds8b, "D_Students");
            //Label16.Text = ds8b.Tables[0].Rows[0]["bal"].ToString();

            cmd8c.CommandText = "select count(*) as bal from D_Lms";
            cmd8c.Connection = con;
            sda8c.SelectCommand = cmd8c;
            sda8c.Fill(ds8c, "D_Lms");
            //Label17.Text = ds8c.Tables[0].Rows[0]["bal"].ToString();
            cmd8d.CommandText = "select count(*) as bal from D_Classes";
            cmd8d.Connection = con;
            sda8d.SelectCommand = cmd8d;
            sda8d.Fill(ds8d, "D_Classes");
            //Label18.Text = ds8d.Tables[0].Rows[0]["bal"].ToString();
            cmd8e.CommandText = "select count(*) as bal from D_Routes";
            cmd8e.Connection = con;
            sda8e.SelectCommand = cmd8e;
            sda8e.Fill(ds8e, "D_Classes");
            //Label19.Text = ds8e.Tables[0].Rows[0]["bal"].ToString();
        ...
        {
            //Bindchart();
            //string jjjj = "select top 5 Concat(a.FirstName,' ',a.LastName) as Name,a.Gender,b.FClass,Concat('~/Students/',a.Photo,a.filetype) as Photo,a.Capture_Date as Date from d_students a,d_classes b where a.class = b.classID order by a.SID desc";
            //GridViewBind2(jjjj);
    ...
    //    }
    //}
    protected void GridViewBind4(string query)
    {
        try
        {
            string constring = ConfigurationManager.ConnectionStrings["SchoolMaster"].ConnectionString;
